<template>
  <div class="container">
    <div class="section links">
      <Nav />
    </div>
    <div class="section details">
      <h1 class="title">Order of Service</h1>
      <p><em>Saturday 5th June 2021</em>The plan is to live-stream the service so everyone who wants to attend will be able to do so virtually. We will also hold a celebration of light and music offerings after the service.</p>
      <p class="highlight">The family and friends wish to express their appreciation for the kindness, sympathy, and support which has been extended to them in their bereavement.</p>
      <h2>The Service <span>14:00 (BST)</span></h2>
      <ul class="fa-ul">
        <ListItem>Music Before Service: Tina Turner - We Don’t Need Another Hero (Thunderdrome)</ListItem>
        <ListItem>Music Before Service: Eminem – I’m Not Afraid</ListItem>
        <ListItem>Service starts at 14:00 </ListItem>
        <ListItem>Entrance music: Duff Kelly – Holy Lady</ListItem>
	<ListItem>Entrance music: Nahko and Medicine For The People – Love Letters To God</ListItem>
        <ListItem>Welcome and Opening Address</ListItem>
        <ListItem>Music: Amazing Grace (My Chains Are Gone)</ListItem>
        <ListItem>Tributes & Memories</ListItem>
        <ListItem>A Poem read by his sister Susan</ListItem>
        <ListItem>Friend Tribute: David</ListItem>
        <ListItem>Eulogy by Mike</ListItem>
        <ListItem>Montage and Reflection</ListItem>
        <ListItem>Closing Remarks and Committal</ListItem>
	<ListItem>Exit Music: Dedication by David's Son and Daughter (Chris De Burgh - Don't Pay The Ferryman)</ListItem>
        <ListItem>Committal Music: Xavier Rudd - Spirit Bird</ListItem>
        <ListItem>Planting a Cherry Tree in accordance with David's wishes</ListItem>
	<ListItem>Music whilst planting tree: Nahko and Medicine For The People - Dear Brother</ListItem>
	<ListItem>Celebration of Light and Music dedications follow this service.</ListItem>
	<ListItem>Follow us after the service for music dedications on <a href="https://bit.ly/barondavidsplaylist">Baron David's playlist</a>
      </ul>
    </div>
  </div>
</template>

<script>
export default {}
</script>

<style lang="scss">
</style>
